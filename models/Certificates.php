...
class Certificates extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('student_id, bd, gst, other', 'required'),
			array('student_id', 'numerical', 'integerOnly'=>true),
			array('bd, gst', 'length', 'max'=>100),
			
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('certificates_id, student_id, bd, gst, other', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('certificates_id',$this->certificates_id);
